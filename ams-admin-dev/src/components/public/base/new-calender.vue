<template>
  <div class="new-calender" :class="[type==='light' && 'new-calender-light']">
    <div
      class="calender-title flex a-center j-between flex-row"
      :class="[type==='light' && 'light']"
    >
      <i class="el-icon-arrow-left" @click="actionHandle('prevMonth')" />
      <span class="time">{{ nowYear }}年 {{ nowZhMonth }}月</span>
      <i class="el-icon-arrow-right" @click="actionHandle('nextMonth')" />
    </div>
    <div class="calender-content" :class="[type==='light' && 'light']">
      <div class="t-header flex a-center flex-row j-between">
        <div v-for="(item,index) in weekList" :key="index*0.1+0.11" class="tb t-box">{{ item }}</div>
      </div>
      <div class="t-body flex a-center flex-row j-start">
        ...
        <div v-for="j in lastWeekDay" :key="j*0.4+0.44" class="th th-color1 t-box">{{ j }}</div>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    selectDate(item) {
      const nowMonth = this.nowMonth < 10 ? '0' + this.nowMonth : this.nowMonth
      const nowDay = item < 10 ? '0' + item : item
      const result = `${this.nowYear}/${nowMonth}/${nowDay}`
    ...
    .light {
      ...
      .today-text {
        ...
        color: #D70010  !important;
         border-bottom: 1px solid #D70010 !important;
      }
    }
  }
  .th{
