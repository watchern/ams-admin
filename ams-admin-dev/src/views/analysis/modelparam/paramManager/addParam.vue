  ...
  <div class="tab-content">
    <div  id="basicInfo">
      <el-form id="ammParamAdd" :model="form"  class="detail-form" style="height:62vh; overflow:auto;">
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="参数名称" prop="paramName">
              <el-input v-model="form.paramName" id="paramName" placeholder="参数名称" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
            <el-form-item label="数据类型" prop="dataType">
              <el-select v-model="form.dataType" @change="changeValue" id="dataType" name="dataType">
                <el-option value="" key="" label="请选择"></el-option>
                <el-option v-for="dataType in this.dataTypes" :value="dataType.codeValue"  :key="dataType.codeValue" :label="dataType.codeName"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <div id="dataLengthInterVal" v-show="this.isShowElement.dataLengthInterValShow">
            <el-form-item label="参数值长度" prop="dataLength">
              <el-input v-model.number="form.dataLength" id="dataLength" placeholder="参数值长度" autocomplete="false" />
            </el-form-item>
        </div>
<!--        <div class="form-group" id="timeInterval" v-show="this.isShowElement.timeIntervalShow">
            ...
            <el-col :span="24">
              <el-form-item label="形式类型">
                <el-radio-group v-model="form.formalType">
                  <el-radio id="choiceType" name="choiceType" :label="'1'">单选</el-radio>
                  <el-radio id="choiceType1" name="choiceType" :label="'0'">多选</el-radio>
                </el-radio-group>
              </el-form-item>
            </el-col>
          </el-row>
        ...
        <el-row>
          <el-form-item label="说明">
              <el-input name="description" v-model="form.description" type="textarea" id="description" autocomplete="off" placeholder="说明"/>
          </el-form-item>
        </el-row>
      </el-form>
      <el-dialog
...
export default{
  ...
  created() {
    ...
    this.maxIntervalUnits = this.setJsonSelectValue("maxIntervalUnit","/paramController/loadDict?codeType=maxIntervalUnit")
    this.dataTypes = this.setJsonSelectValue("dataType","/paramController/loadDict?codeType=dataType");
    this.inputTypeValueStr  = this.setJsonSelectValue("inputType","/paramController/loadDict?codeType=inputType","","");
  },
  mounted() {
            ...
            if (data.paramChoice) {
              ...
              if (data.paramChoice.ammParamChoiceUuid) {
                if (data.paramChoice.ammParamOptionsList && data.paramChoice.ammParamOptionsList.length > 0) {
                  that.tabsigns = 0;
                  for (var i = 0; i < data.paramChoice.ammParamOptionsList.length; i++) {
                    if(i == 0){
                      that.defaultExistsCustomStaticValues.names = data.paramChoice.ammParamOptionsList[i].optionsName
                      that.defaultExistsCustomStaticValues.values = data.paramChoice.ammParamOptionsList[i].optionsVal
                      that.defaultExistsCustomStaticValues.uuid = data.paramChoice.ammParamOptionsList[i].ammParamOptionsUuid
                    }
                    else{
                      //整理数据对象
                      that.customStaticValues.push({
                        names:data.paramChoice.ammParamOptionsList[i].optionsName,
                        values:data.paramChoice.ammParamOptionsList[i].optionsVal,
                        uuid:data.paramChoice.ammParamOptionsList[i].ammParamOptionsUuid
                      })
                    }
                  ...
                  if (data.inputType == 'treeinp') {
                    that.form.paramChoice.optionsSql = data.paramChoice.optionsSql
                  } else {
                    that.activeName = "SQL"
                    that.form.paramChoice.optionsSqlLine = data.paramChoice.optionsSql
                  }
                }
              }
            }
