...
Graph.prototype.getPageSize = function() {
	//这原来是获得页面大小，该大小指默认A4纸张大小，现在注释掉，改为以画布大小为准
	try {
		this.pageFormat = new mxRectangle(0, 0, this.container.offsetWidth - 20, this.container.offsetHeight - 20);
	} catch(e){}
...
Graph.prototype.updatePlaceholders = function() {
	...
	for(var key=0;key<this.model.cells.length;key++) {
 ...
 * Up arrow.
 */
HoverIcons.prototype.arrowFill = '#3e6f96';
...
HoverIcons.prototype.init = function() {
	...
	this.arrowDown = this.createConnetionBtn(this.triangleDown, mxResources.get('plusTooltip'));
	...
	this.graph.addMouseListener({
		...
		mouseUp: mxUtils.bind(this, function(sender, me) {
			//修改被引用节点 引起 引用节点的变化
			try {
				for(var key=0;key<graph.model.cells.length;key++) {
					if(graph.model.cells[key].vertex && Boolean(graph.model.cells[key].referedNodeId)) {
						graph.model.cells[key].value = graph.model.cells[graph.model.cells[key].referedNodeId].value;
						var $refered = $("image.cell-img[nodeId="+ graph.model.cells[key].referedNodeId +"]");
						var $referedFo = $refered.parent().next().children();
						var $last = $("image.cell-img[nodeId="+ graph.model.cells[key].id +"]");
						var $lastFo = $last.parent().next().children();
						$lastFo.html($referedFo.html());
						//重置位移
 ...
 * 创建连接按钮
 */
HoverIcons.prototype.createConnetionBtn = function(img, tooltip) {
	var arrow = null;
...
//			graph.currentEdgeStyle['edgeStyle'] = "orthogonalEdgeStyle";
			...
			graph.currentEdgeStyle['strokeColor'] = "#3E6F96";
			graph.currentEdgeStyle['strokeWidth'] = "2";
			...
			if(libs != null) {
				if(mxStencilRegistry.packages[basename] == null) {
					for(var i = 0; i < libs.length; i++) {
						...
						if(fname.toLowerCase().substring(fname.length - 3, fname.length) == '.js') {
							try {
