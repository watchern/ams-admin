 ...
 */
Editor = function(themes, model, graph) {
...
function Dialog(editorUi, elt, w, h, modal, closable, onClose, noScroll, transparent) {
	...
	var top = Math.max(1, Math.round((dh - h) / 3));
...
/*	//dialog右上角关闭按钮
	...
	this.resizeListener = mxUtils.bind(this, function() {
		...
		top = Math.max(1, Math.round((dh - h) / 3));
...
(function() {
	...
	mxGraphView.prototype.validateBackgroundPage = function() {
		...
		//document.getElementById("geBackgroundPage").style.backgroundImage = image;
		return;
		//去画布 -end
		...
		if(graph.container != null && !graph.transparentBackground) {
			if(graph.pageVisible) {
				...
				if(this.backgroundPageShape == null) {
					...
					if(firstChild != null) {
						this.backgroundPageShape = new mxRectangleShape(bounds, '#ffffff', '#ffffff');
						...
						//[删掉]graph.container.insertBefore(this.backgroundPageShape.node, firstChild);
						this.backgroundPageShape.node = document.getElementById("geBackgroundPage");
