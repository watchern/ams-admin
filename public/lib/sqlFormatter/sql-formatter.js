	...
	var Tokenizer = function () {
	    ...
	    Tokenizer.prototype.getTokenOnFirstMatch = function getTokenOnFirstMatch(_ref3) {
	        ...
	        if (matches) {
	        	...
	        	//modify by mhc 如果为自定义控件，则跳过
				if(_ref3.type==_tokenTypes2["default"].WORD) {
					var widgetTableRegex = /^\{\$(.+?)\$\}/g;
					var widgetParamRegex = /^\{\#(.+?)\#\}/g;
					matches = input.match(widgetTableRegex)||input.match(widgetParamRegex);
					if (matches) {
						return {type: type, value: matches[0]};
					}
				}
			}
	    };
