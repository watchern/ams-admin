...
(function(mod) {
  ...
  Completion.prototype = {
    ...
    pick: function(data, i) {
      ...
      if(completion.className.indexOf("table")>-1){
        var realText = "{" + getText(completion) + "}";
      }
      if (completion.hint) completion.hint(this.cm, data, completion);
      else this.cm.replaceRange(realText, completion.from || data.from,
          completion.to || data.to, "complete");
      if(completion.className.indexOf("table")>-1){
        // var newTo = (completion.from || data.from);
        // newTo.ch=(completion.from || data.from).ch + getText(completion).length;
        //获取最后焦点
        // console.log(data.from);
        editor.markText(completion.from || data.from, editor.getCursor(),{//设置字体样式（前两个参数表示设置的初始位置和结束位置，最后的是用什么类型来替换）
          replacedWith: SQLEditor.getTableStyle(getText(completion))
          ,className: "paramBlock1"
        });
      }
