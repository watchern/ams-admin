...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
  ...
  ,TPL_HEADER = function(options){
    ...
    options = options || {};
    return ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table kssj-table"  '+ (typeof(options.style)=='undefined'?'':options.style)
      ...
      ,'{{# layui.each(d.data.cols, function(i1, item1){ }}'
        ...
        ,'{{# layui.each(item1, function(i2, item2){ }}'
          ...
          ,'<th {{# if((item2.field||i2)!="0"){ if(checkPercent(item2.width)){ }} style="width:{{item2.width}}" {{# }else{ }}style="width:{{item2.width}}px"{{# } } }} {{# if((item2.field||i2)=="0"){ }}style="width:48px"{{# } }}  data-field="{{ item2.field||i2 }}" {{# if(item2.minWidth){ }}data-minwidth="{{item2.minWidth}}"{{# } }} '+ rowCols +' {{# if(item2.unresize){ }}data-unresize="true"{{# } }}>'
  ...
  //tbody区域模板
  ,TPL_BODY = ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table kssj-table" style="width:100%;table-layout:fixed" '
    ,'{{# if(d.data.skin){ }}lay-skin="{{d.data.skin}}"{{# } }} {{# if(d.data.size){ }}lay-size="{{d.data.size}}"{{# } }} {{# if(d.data.even){ }}lay-even{{# } }}>'
    ,'<tbody></tbody>'
  ,'</table>'].join('')
  ,TPL_BODY_FIX = ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table kssj-table"  '
    ,'{{# if(d.data.skin){ }}lay-skin="{{d.data.skin}}"{{# } }} {{# if(d.data.size){ }}lay-size="{{d.data.size}}"{{# } }} {{# if(d.data.even){ }}lay-even{{# } }}>'
    ,'<tbody></tbody>'
  ,'</table>'].join('')
  ...
  //,TPL_MAIN = ['<div class="layui-form layui-border-box {{d.VIEW_CLASS}}" lay-filter="LAY-table-{{d.index}}" style="{{# if(checkRate(d.data.width)){ }}width:{{d.data.width}}px;{{# }else{ }} width:{{d.data.width}};{{# } }}{{# if(d.data.height){ }}height:{{d.data.height}}px;{{# } }}">'
    ...
    ,'<div class="layui-table-box">'
      ...
      ,'<div class="layui-table-header">'
        ,TPL_HEADER({style:'style="width:100%;table-layout:fixed"'})
      ,'</div>'
  ...
  Class.prototype.pullData = function(curr, loadIndex){
	var pageNum = $("#pageNum").val();
	 //此处是已办列表跳转专用---点击标题查看后返回到指定的页数----2018.6.7---start------
	 if(typeof(pageNum)!="undefined"){
	   if(pageNum!=""){
		  curr = pageNum;
	  }
     }
	 //此处是已办列表跳转专用---点击标题查看后返回到指定的页数----2018.6.7----end-----
    var that = this
    ,options = that.config
    ...
    if(options.url){ //Ajax请求
      ...
      $.ajax({
        ...
        ,url: options.url
        ,cache:false//在get请求下，浏览器缓存会出现问题
      ...
      layui.each(data, function(i1, item1){
        ...
        that.eachCols(function(i3, item3){
          ...
          var td = ['<td data-field="'+ field +'" '+ function(){
            ...
            if(field!='0'){
            	if(checkPercent(item3.width)){
                	attr.push('style="width:'+ item3.width +'"'); //修改添加了宽度百分比
            	}else{
                	attr.push('style="width:'+ item3.width +'px"'); //修改添加了宽度px
            	}
            }
            if(field=='0') attr.push('style="width:48px"'); //修改添加了宽度百分比
            return attr.join(' ');
          }() +'>'
