...
QueryBuilder.prototype.createRuleFilters = function(rule, container, defaultDir) {
    ...
    $(qbSettings.settings.dataResType).each(function(index) {
    	rule.$el.find(container + " .table-cont").append('<option value="'+ qbSettings.settings.dataResType[index].id +'" restype="'+ qbSettings.settings.dataResType[index].type +'" '+ (rule.data && rule.data["control" + defaultDir] == qbSettings.settings.dataResType[index].id? "selected=\"selected\"": "") +'>'+ qbSettings.settings.dataResType[index].value +'</option>');
    });
    

    this.createRuleFields(container, rule, 
    		rule.data? rule.data["control" + defaultDir]: qbSettings.settings.dataResType[0].id, 
    	defaultDir);
    if(rule.data && rule.data["type" + defaultDir] == "val") {
    	this.createRuleInputText(container, rule, defaultDir);
    } else if(rule.data && rule.data["type" + defaultDir] == "param") {
...
QueryBuilder.prototype.createRuleFields = function(container, rule, dataResId, defaultDir) {
	var fields = JSON.parse(parent.graph.model.cells[dataResId].nodeOutputData).field;
	...
	this.settings.fields[dataResId] = newField;
