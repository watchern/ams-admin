...
QueryBuilder.selectors = {
    ...
    rule_operator_oper:   '.rule-operator-container [oper$=_operator]',
...
QueryBuilder.DEFAULTS = {
    ...
    templates: {
        ...
        filterControlSelect: null,
        ruleValueControlSelect: null
    },
...
QueryBuilder.prototype.addRule = function(parent, data, flags) {
    ...
    model.filter = {
    	...
    	default_operator: (model.data? model.data.operator: null)
    };
    
    this.createRuleFilters(model, QueryBuilder.selectors.filter_container, "Left");
    this.createRuleOperators(model);
    this.createRuleFilters(model, QueryBuilder.selectors.value_container, "Right");
 ...
 * @private
 */
QueryBuilder.prototype.createRuleFilters = function(rule, container, defaultDir) {
    /**
     ...
     */
	var qbSettings = this;
	var $filterControlSelect = $(this.getRuleFilterControlSelect());
	$filterControlSelect.on("change", function() {
		switch ($(this).find("option:selected").attr("restype")) {
			case "data_res":
				qbSettings.createRuleFields(container, rule, $(this).val());
				break;
			case "val":
				qbSettings.createRuleInputText(container, rule);
				break;
			case "param":
				qbSettings.createRuleParam(container, rule);
				break;
		}
		
	});
	
	rule.$el.find(container).html($filterControlSelect);
    
    if("Left" == defaultDir) {
        $(qbSettings.settings.dataResType).each(function(index) {
        	rule.$el.find(container + " .table-cont").append('<option value="'+ qbSettings.settings.dataResType[index].id +'" restype="'+ qbSettings.settings.dataResType[index].type +'" '+ (rule.data? rule.data["control" + defaultDir]: null == qbSettings.settings.dataResType[index].id? "selected=\"selected\"": "") +'>'+ qbSettings.settings.dataResType[index].value +'</option>');
        });
    	
    	this.createRuleFields(container, rule, qbSettings.settings.dataResType[0].id, defaultDir);
    } else if("Right" == defaultDir) {
    	rule.$el.find(container + " .table-cont").append('<option value="value" restype="val" '+ (rule.data? rule.data["control" + defaultDir]: null == 'val'? "selected=\"selected\"": "") +'>值</option>');
    	rule.$el.find(container + " .table-cont").append('<option value="param" restype="param" '+ (rule.data? rule.data["control" + defaultDir]: null == 'param'? "selected=\"selected\"": "") +'>参数</option>');
    	this.createRuleInputText(container, rule, defaultDir);
    	if(rule.data && rule.data["type" + defaultDir] == "param") {
        	this.createRuleParam(container, rule, defaultDir);
        }
    }
 ...
 * 当所选为“表”时
 */
QueryBuilder.prototype.createRuleFields = function(container, rule, dataResId, defaultDir) {
	var fields = this.settings.fields[dataResId];
	var $filterSelect = $(this.getRuleFilterSelect(rule, fields, dataResId));
	
	rule.$el.find(container).find(".table-field").remove();
	rule.$el.find(container).append($filterSelect);
	if(rule.data) {
		rule.$el.find(container).find(".table-field option [value='"+ rule.data['field' + defaultDir] +"']").attr("selected", "selected");
	}
}
/**
 * 当所选为“值”时
 */
QueryBuilder.prototype.createRuleInputText = function(container, rule, defaultDir) {
	var $cont = rule.$el.find(container);
	var selName = $cont.find(".table-field").prop("name") + "_right";
	$cont.find(".table-field").remove();
	$cont.append('<input class="form-control table-field" type="text" name="'+ selName +'" value="'+ (rule.data && undefined != rule.data["field" + defaultDir]? rule.data["field" + defaultDir]: "") +'" />');
}
/**
 * 当所选为“参数”时
 */
QueryBuilder.prototype.createRuleParam = function(container, rule, defaultDir) {
	var $cont = rule.$el.find(container);
	$cont.find(".table-field").remove();
	$cont.append('<button type="button" class="form-control table-field">点击添加参数</button>');
}
...
QueryBuilder.prototype.createRuleOperators = function(rule) {
    ...
    var operators = rule.filter.operators;//this.getOperators(rule.filter);
    ...
    rule.$el.find(QueryBuilder.selectors.rule_operator_oper).val(rule.operator.type);
...
QueryBuilder.prototype.getRules = function(options) {
    ...
    var out = (function parse(group) {
        ...
        group.each(function(rule) {
            ...
            var curRuleElLeft = $("#" + rule.$el[0].id).find(QueryBuilder.selectors.filter_container);
            var curRuleOperator = $("#" + rule.$el[0].id).find(QueryBuilder.selectors.operator_container);
            var curRuleElRight = $("#" + rule.$el[0].id).find(QueryBuilder.selectors.value_container);
            
            var ruleData = {
                    controlLeft: $(curRuleElLeft).find(".table-cont").val(),
                    typeLeft: $(curRuleElLeft).find(".table-cont option:selected").attr("restype"),
                    fieldLeft: self.parseFilter(curRuleElLeft),
                    operator: $(curRuleOperator).find("select").val(),
                    controlRight: $(curRuleElRight).find(".table-cont").val(),
                    typeRight: $(curRuleElRight).find(".table-cont option:selected").attr("restype"),
                    fieldRight: self.parseFilter(curRuleElRight)
                };
...
QueryBuilder.prototype.parseFilter = function(curRuleEl) {
	switch ($(curRuleEl).find(".table-cont option:selected").attr("restype")) {
		case "data_res":
			return $(curRuleEl).find(".table-field").val();
		case "val":
			return $(curRuleEl).find(".table-field").val();
		case "param":
			return "{#" + $(curRuleEl).find(".table-field").val() + "#}";
    ...
    (function add(data, group) {
        ...
        data.rules.forEach(function(item) {
            ...
            if (item.rules !== undefined) {
                ...
                model = self.addRule(group, item, self.parseRuleFlags(item));
                if (model === null) {
                    return;
                }
 ...
 * 由setRules改动
 */
QueryBuilder.prototype.setCurRules = function(data, options) {
    options = $.extend({
        allow_invalid: false
    }, options);
...
<div id="{{= it.rule_id }}" class="rule-container"> \
  ...
  {{? it.settings.display_errors }} \
    <div class="error-container"><i class="{{= it.icons.error }}"></i></div> \
  {{?}} \
  <div class="rule-filter-container border-radius-group"></div> \
  <div class="rule-operator-container"></div> \
  <div class="rule-value-container border-radius-group"></div> \
</div>';

QueryBuilder.templates.filterControlSelect = ' \
	<select class="form-control table-cont"></select> \
';

QueryBuilder.templates.filterSelect = '\
<select class="form-control table-field" name=""> \
  {{~ it.fields: field }} \
    <option value="{${{= it.dataResId }}$}.{{= field.name }}" {{? field.icon}}data-icon="{{= field.icon}}"{{?}} >{{= it.translate(field.alias) }}</option> \
  {{~}} \
</select>';
...
{{?}} \
{{ var optgroup = null; }} \
<select class="form-control {{? it.operators.length === 1 }}hide{{?}}" oper="{{= it.rule.id }}_sel_operator"> \
 ...
 * @private
 */
QueryBuilder.prototype.getRuleFilterControlSelect = function() {
    var h = this.templates.filterControlSelect({});
     ...
     * @returns {string}
     */
    return this.change('getRuleFilterControlSelect', h);
};
QueryBuilder.prototype.getRuleFilterSelect = function(rule, fields, dataResId) {
     ...
     * @returns {string}
     */
    return this.change('getRuleFilterSelect', h, rule, fields);
};
        ...
        var sql = (function parse(group) {
            ...
            group.rules.forEach(function(rule) {
                if (rule.rules && rule.rules.length > 0) {
                     ...
                     * @returns {string}
                     */
                    var ruleExpression = rule.fieldLeft + " " + sqlFn(rule.fieldRight);
                     ...
                     * @returns {string}
                     */
                    parts.push(self.change('ruleToSQL', ruleExpression, rule, "", sqlFn));
                }
            });
