...
export default {
  ...
  methods: {
    ...
    detail() {
      const selectObj = this.$refs.graphListTable.selection
    ...
    edit() {
      ...
      if (selectObj.length !== 1) {
        ...
        const graphType = selectObj[0].graphType
        const publicType = selectObj[0].publicType
    ...
    deleteGraph() {
      ...
      if (selectObj.length === 0) {
        ...
        this.$confirm('确定删除图形?', '提示', {
          ...
          const obj = this.getUuidsANdNames(selectObj)
    ...
    share() {
      ...
      if (selectObj.length === 0) {
        ...
        const curPersonUuid = this.$store.getters.personuuid
        const personObj = { personUuids: '2c91808573e740e001744d54e2800005', personNames: '张闯1' }// 模拟选择的人员结果信息，待集成方法
        if (personObj.personUuids.indexOf(curPersonUuid) > -1) {
          this.$message({ type: 'warning', message: '不可将个人图形分享给自己' })
        } else {
          const graphObj = this.getUuidsANdNames(selectObj)// 获取选择的图形信息
    ...
    cancelShare() {
      ...
      if (selectObj.length !== 1) {
        ...
        const graphObj = { graphUuid: selectObj[0].graphUuid, graphName: selectObj[0].graphName }
        const personObj = { personUuids: '2c91808573e740e001744d54e2800005', personNames: '张闯1' }// 模拟选择的人员结果信息，待集成方法
    ...
    getUuidsANdNames(selectObj) {
      ...
      for (let i = 0; i < selectObj.length; i++) {
        const curUuid = selectObj[i].graphUuid
        const curName = selectObj[i].graphName
        if (graphUuids === '') {
          graphUuids = curUuid
          graphNames = curName
        } else {
    ...
    stasusFormatter(row, column) {
      const executeStatus = row.executeStatus
      let str = ''
