    ...
    <el-table :key="tableKey" ref="graphListTable" v-loading="listLoading" height="500" :data="dataList" border fit highlight-current-row>
      ...
      <el-table-column label="图形名称" align="left" prop="graphName">
        <template slot-scope="scope">
          <el-link type="primary" @click.native.prevent="detail(scope.row.graphUuid)">
            {{ scope.row.graphName }}
          </el-link>
        </template>
      </el-table-column>
  ...
  methods: {
    ...
    getChooseGraphs() {
      ...
      if (selectObj.length === 0) {
        ...
        const graphObj = this.getUuidsANdNames(selectObj)
    ...
    dateFormatter(row, column) {
      ...
      if (datetime && datetime.trim().length > 0) {
        ...
        const month = this.convertData(dateMat.getMonth() + 1)
        const day = this.convertData(dateMat.getDate())
        const hh = this.convertData(dateMat.getHours())
        const mm = this.convertData(dateMat.getMinutes())
        const ss = this.convertData(dateMat.getSeconds())
