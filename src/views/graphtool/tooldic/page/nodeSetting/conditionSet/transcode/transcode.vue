  ...
  <div id="transcode">
    <form class="layui-form">
      <div class="layui-form-item">
        <label class="layui-form-label">上一节点名称：<span /></label>
    ...
    <table id="main_table" class="table table-striped">
      ...
      <tbody>
        <tr v-for="(item,index) in items" :key="item.key">
          ...
          <td>
            <SelectTransCode ref="SelectTransCode" :transuuid.sync="scope.row.transRuleUuid" />
            <el-select v-model="item.transRuleUuid" filterable>
              <el-option v-for="ruleObj in ruleArr" :key="ruleObj.transRuleUuid" :value="ruleObj.transRuleUuid">{{ ruleObj.ruleName }}</el-option>
            </el-select>
            <button class="chooseRule layui-btn" @click="previewTransCode(item.transRuleUuid)">查看</button>
            <!--<input type="hidden" class="ruleId" v-model="item.ruleId"/>-->
            <!--<input type="text" class="ruleName layui-input" v-model="item.ruleName" readonly="readonly"/>-->
            <!--&nbsp;<button class="chooseRule layui-btn" @click="chooseRule(index)">选择</button>-->
          </td>
          <td>
...
export default {
  ...
  created() {
    const loading = $('#transcode').mLoading({ 'text': '正在加载转码数据，请稍后……', 'hasCancel': false })
    getTransCodeList().then(response => {
      loading.destroy()
      if (response.data == null) {
        alertMsg('提示', '转码数据获取失败', 'error')
      } else {
        ruleArr = response.data
      }
    })
  },
  mounted() {
  ...
  methods: {
    init() {
      ...
      const parentIds = graph.nodeData[graph.curCell.id].parentIds
