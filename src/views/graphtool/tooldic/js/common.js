...
export function data_filter(type, name, nodeName) {
  ...
  var obj = verifyPreNodes(type, curNodeId)
  if (obj.isError) {
    alertMsg('错误', obj.message, 'error')
    return
  }
  if (!settingVerify(type)) {
    return
  }
  var url = ''
  if (type === 'relation') {
...
function settingCallBack(curNodeId, type, layero) {
  ...
  // 验证数据融合节点输出列信息
  if (type === 'union' && !verifyUnionOutputColumn(layero)) {
    return false
  }
...
function createParamNodeHtml(nodeIdArr) {
  ...
  for (var i = 0; i < nodeIdArr.length; i++) {
    ...
    var paramsSetting = graph.nodeData[nodeIdArr[i]].paramsSetting// 参数设置信息
 ...
 * */
function executeNode(notExecuteNodeIdArr) {
  var executeCellId = graph.curCell.id
  var obj = {
    'verify': true,
    'message': ''
  }
  if (!notExecuteNodeIdArr || notExecuteNodeIdArr.length === 0) {
    if (!executeVerify()) {
      return
    }
  ...
  var hasRunningTaskTable_CallBack = function() {
    ...
    if (graph.openType === 2) { // 权限环境才会检测表空间大小
      ...
      if (hasCreateTableNode(notExecuteNodeIdArr)) { // 如果有，则判断表空间是否足够
        obj = getSpaceSize()
        if (!obj.verify) {
          alertMsg('提示', obj.message, 'info')
          return
  ...
  if (graph.openType === 2) { // 权限环境才会检测跑批数据表
    ...
    hasRunningTaskTable(notExecuteNodeIdArr, hasRunningTaskTable_CallBack, false)
  } else {
...
function executeNode_callback(notExecuteNodeIdArr) {
  ...
  var dataParam = {
    ...
    'nodeIdList': notExecuteNodeIdArr,
    'nodeData': JSON.stringify(graph.nodeData)
  }
  $.ajax({
    url: contextPath + '/graphEditor/executeNodeSql',
    type: 'post',
    data: dataParam,
    dataType: 'json',
    ...
    complete: function(xhr) {
      if (xhr.statusText === 'timeout') {				// 如果出现超时，每隔一分钟从缓冲表中查询与当前图形化有关的执行中节点的执行结果信息
        getExecuteNodeInfo(null, executeId, notExecuteNodeIdArr, true)
      } else {
        var historyNodeName = ''
        $('ul.layui-tab-title li:eq(1)').click()
        $('#sysInfoArea').html(xhr.responseJSON.message)
        // 循环所有节点变更执行状态有变化的节点执行状态信息
        nodeCallBack(notExecuteNodeIdArr, xhr.responseJSON.nodeData, executeId)
        // 如果执行的是未配置的结果表
        if (!isSet) {
          graph.curTableName = graph.nodeData[parentIds[0]].nodeInfo.resultTableName
          historyNodeName = graph.nodeData[parentIds[0]].nodeInfo.nodeName
        } else {
          graph.curTableName = graph.nodeData[executeCellId].nodeInfo.resultTableName
        ...
        if (graph.nodeData[executeCellId] && graph.nodeData[executeCellId].nodeInfo && graph.nodeData[executeCellId].nodeInfo.nodeExcuteStatus === 3) {
          ...
          if (!graph.nodeData[executeCellId].isSet && optType === 'newNullNode') {
            optType = graph.nodeData[parentIds[0]].nodeInfo.optType
          }
          viewData(graph.curTableName, optType, true, type)
          graph.curCellId = executeCellId
 ...
 * */
function nodeCallBack(executeNodeArr, executeNodeData, executeId) {
