 ...
 */
function initParamHtml_Common(paramObj, selectNum, selectTreeNum) {
  var obj = {
    'htmlContent': '',
    'isError': false,
    'message': ''
  ...
  var title = paramObj.ammParamChoice.allowedNull === 0 ? '不可为空' : '可为空'
  obj.spanHtml = paramObj.ammParamChoice.allowedNull === 0 ? '<div style="color: red;display: inline-block;font-weight: bold;font-size: 20px;">*</div>' : ''
  switch (paramObj.inputType) {
    case 'lineinp':// 下拉列表
      ...
      var paramSql = paramObj.ammParamChoice.optionsSql
      // 备选sql为空的情况下 取静态的option值
      if (!paramSql) {
        $.each(paramObj.ammParamChoice.ammParamOptionsList, function(i, v) {
        ...
        if (paramSql !== '') {
          ...
          if (typeof paramObj.defaultVal !== 'undefined') { // 如果有该参数默认值，则直接执行备选SQL加载初始化数据
            $.ajax({
              ...
              method: 'POST',
              cache: false,
              data: { 'sql': strEncryption(paramObj.ammParamChoice.optionsSql) },
              dataType: 'json',
              async: false,
              success: function(e) {
                if (e.isError) {
      ...
      obj.htmlContent += "<div id='" + divId + "' title='" + title + "'  data-id='" + paramObj.ammParamUuid + "' data-name='" + paramObj.paramName + "' data-choiceType='" + paramObj.ammParamChoice.choiceType + "'" +
                ...
                (typeof paramObj.ammParamChoice.allowedNull !== 'undefined' ? " data-allowedNull='" + paramObj.ammParamChoice.allowedNull + "' " : '') +
...
function selectShow_common(idStr, ind, paramName, sql, choiceType, paramArr, dataArr, initDataArr) {
  ...
  try {
    ...
    $('.selectParam').each(function(i, v) {
      ...
      for (var t = 0; t < paramArr.length; t++) {
        if (paramArr[t].paramId === curParamId) {
          var selectParamXs = xmSelect.get('#selectParam' + i, true)// 获取当前下拉框的实体对象
          var selectedObjArr = selectParamXs.getValue()// 获取选中的参数值集合
    ...
    $('.selectTreeParam').each(function(j, v) {
      var curParamId = $(this).attr('data-id')// 参数ID
      for (var t = 0; t < paramArr.length; t++) {
        if (paramArr[t].paramId === curParamId) {
          var selectTreeParamXs = xmSelect.get('#selectTreeParam' + j, true)// 获取当前下拉框的实体对象
          var selectedObjArr = selectTreeParamXs.getValue()// 获取选中的参数值集合
          if (selectedObjArr && selectedObjArr.length > 0) {
            var associatedParamCol = paramArr[t].associatedParamCol// 被关联参数值（对应的列名称）
            if (choiceType == 1) { // 单选
              sqlWhereStr += ' and ' + associatedParamCol + "='" + selectedObjArr[0].value + "'"
            } else { // 多选
              var valueStr = ''
              for (var k = 0; k < selectedObjArr.length; k++) {
                valueStr += "'" + selectedObjArr[k].value + "',"
              }
              if (valueStr !== '') {
                valueStr = valueStr.substring(0, valueStr.length - 1)
              }
...
function selectHide_common(associatedParamIdArr) {
  $('.selectParam').each(function(i, v) {
    ...
    if ($.inArray(curParamId, associatedParamIdArr) != -1) {
      var selectParamXs = xmSelect.get('#selectParam' + i, true)// 获取当前下拉框的实体对象
      var selectedObj = selectParamXs.getValue()// 获取选中的参数值
      if (selectedObj && selectedObj.length > 0) {
        selectParamXs.setValue([])// 清空选中值
      }
    }
  })
  $('.selectTreeParam').each(function(j, v) {
    ...
    if ($.inArray(curParamId, associatedParamIdArr) != -1) {
      var selectTreeParamXs = xmSelect.get('#selectTreeParam' + j, true)// 获取当前下拉框的实体对象
      var selectedObj = selectTreeParamXs.getValue()// 获取选中的参数值
      if (selectedObj && selectedObj.length > 0) {
        selectTreeParamXs.setValue([])// 清空选中值
      }
    }
  })
}
 ...
 * @author JL
 */
function sortParamArr(paramAarr) {
  var index = 0
