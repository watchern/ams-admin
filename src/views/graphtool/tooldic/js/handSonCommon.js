...
var handSonTableFun = function(setting) {
  ...
  var connGraph = !!setting.connGraph
  var viewAllData = !!setting.viewAllData
  // end
  var columnInfo = []; var data = []; var columns = []; var mergeCells = []
  ...
  var h_setting = {
    ...
    wordWrap: true,
    sortDataByCurPage: !!viewAllData, // 是否启用插件自带排序（自定义属性），当预览全部数据时对当前页数据进行前台排序
...
function defaultMenu(columnInfo, connGraph, relation) {
  ...
  var mainMenu = {
    callback: function(key, options, clickEvent) {
      ...
      var cellData = false; var calculate = false
      var currColumnName = ''; var currColumnType = ''
      var cellColArr = []; var cellRowArr = []
      if (options.length == 1) {
      ...
      if (cellData) {
        ...
        var val = 0; var dataArr = []; var longString = ''; var shortString = ''
