import { saveGraphInterface } from '@/api/graphtool/graphList'
...
export function getExecuteNodeInfo(graphUuid, executeId, executeIdArr, refreshHistory) {
  var obj = {}; var len = executeIdArr.length
  ...
  var executeNodeInfo = function() {
    $.post(contextPath + '/graphEditor/getExecuteNodeInfo', obj, function(e) {
      if (e != null && e.length > 0) {
        ...
        for (var i = 0; i < e.length; i++) {
          ...
          for (var j = 0; j < executeNodeIds.length; j++) {
        ...
        if (inerval == null) {
          inerval = setInterval(executeNodeInfo, 20000)				// 设置每一分钟查一次
        }
      }
    }, 'json')
  }
