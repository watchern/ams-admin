...
/**
 * 验证数据融合节点的输出列信息
 * @param layero layer.open的弹出层对象
 **/
export function verifyUnionOutputColumn(layero) {
  var newColumnArr = []; var isExsist = false
  ...
  window[layero.find('iframe')[0]['name']].$('.colTr').each(function(i, v) {
    ...
    var ifCk = $(this).find('input[type=checkbox]').prop('checked')
    if (ifCk) {
      count++
    }
    if ($.inArray(newColumn, newColumnArr) === -1) {
      ...
      if (ifCk && window[layero.find('iframe')[0]['name']].$('.colTr:eq(' + index + ')').find('input[type=checkbox]').prop('checked')) {
        isExsist = true						// 有相同的字段别名
        window[layero.find('iframe')[0]['name']].$('.newColumn:eq(' + index + ')').css('border', '1px solid red')
        $(this).find('.newColumn').css('border', '1px solid red')
        return false
      }
    }
  })
  ...
  if (isExsist) {
    alertMsg('错误', '输出字段别名有重复，请修改', 'warning')
    return false
  }
  return true
}
