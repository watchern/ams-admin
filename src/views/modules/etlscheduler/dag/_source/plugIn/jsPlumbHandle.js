...
import mStart from '@/views/modules/etlscheduler/processinstance/start'
import $ from 'jquery'
    ...
    $('#canvas').droppable({
      ...
      drop: function(ev, ui) {
        ...
        $('#canvas').append(rtTasksTpl({
          ...
          taskType: findComponentDownward(self.dag.$root, 'DagChart').dagBarId
        }))
        ...
        if ($(`#${selfId}`).html()) {
          // dag event
          findComponentDownward(self.dag.$root, 'DagChart')._createNodes({
            id: selfId
          })
        }
      }
...
JSP.prototype.tasksContextmenu = function(event) {
  if (this.config.isContextmenu) {
    ...
    const html = [
      `<a href="javascript:" id="startRunning" class="${isOne ? '' : 'disbled'}"><em class="ans-icon-play"></em><span>运行</span></a>`,
      `<a href="javascript:" id="editNodes" class="${isTwo ? 'disbled' : ''}"><em class="ans-icon-edit"></em><span>编辑</span></a>`,
      `<a href="javascript:" id="copyNodes" class="${isTwo ? 'disbled' : ''}"><em class="ans-icon-copy"></em><span>复制</span></a>`,
      `<a href="javascript:" id="removeNodes" class="${isTwo ? 'disbled' : ''}"><em class="ans-icon-trash"></em><span>删除</span></a>`
    ]
    ...
    if (isOne) {
      ...
      $('#startRunning').on('click', () => {
        ...
        store.dispatch('dag/getStartCheck', {
