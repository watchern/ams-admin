  ...
  <div id="fatherDiv">
    <el-container>
      <el-aside class="menu-aside">
        <div v-for="(item, key) in this.detailModels"
             class="aside-title"
             :key="key">
          <el-tooltip class="item"
            ...
            <el-button @click="jump(key)"
                       :class="[key==activeMenu?'tipButton colorButton':'tipButton']">{{ item.modelName }}</el-button>
          </el-tooltip>
        </div>
      </el-aside>
      <el-container>
        <el-header>
          <div>
            ...
            <label style="margin-right: -43px;color:#9B4C4C;margin-left: 10px;margin-left: 15px;cursor: pointer;"
                   @click="modelResultSavePathDialog = true">{{ path }}</label>
            <!--            <a @click="modelResultSavePathDialog = true" style="color: #409eff"
              >编辑</a
            >-->
          </div>
        </el-header>
        <div class="el-main-param"
             id="scrollBox">
          <el-main>
            <div v-for="(item, key) in this.detailModels"
                 :key="key"
              ...
              <!--              ref="paramassembly"-->
              <!--            ></paramDraw>-->
              <paramDrawNew :sql="drawsSql[key]"
                            :arr="drawParamarr[key]"
                            ref="paramassembly"></paramDrawNew>
            </div>
          </el-main>
        </div>
        <el-dialog title="选择模型结果保存路径"
          ...
          <dataTree :data-user-id="dataUserId"
