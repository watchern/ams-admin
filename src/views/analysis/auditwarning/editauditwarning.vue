  ...
  <div class=""
    ...
    <el-dialog title="选择模型列表"
      ...
      <div slot="footer"
        ...
        <el-button type="primary"
                   @click="selectModel">确定</el-button>
      </div>
    </el-dialog>
    <el-tabs v-model="tabShow">
      <el-tab-pane label="基本信息"
                   name="baseInfo">
        <el-form ref="baseDataForm"
                 :rules="rules"
                 class="detail-form"
                 :model="temp"
          ...
          <el-row>
            <el-col :span="11">
              <el-form-item label="预警类型">
                <el-select v-model="temp.warningType"
                           placeholder="请选择预警类型">
                  <el-option label="模型"
                             :value="1"></el-option>
                ...
                <el-row>
                  <el-col :span="19">
                    <el-input readonly="readonly"
                              v-model="this.auditWarningSave.locationName" />
                  </el-col>
                  <el-col :span="2"
                          :offset="1">
                    <el-button type="primary"
                               @click="modelResultSavePathDialog = true">选择</el-button>
                  </el-col>
                </el-row>
              </el-form-item>
            </el-col>
          ...
          <el-row>
            <el-col :span="11">
              <el-form-item label="分配类型">
                <el-select v-model="auditWarningSave.distributionType"
                           @change="distributionTypeChange"
                           placeholder="请选择分配类型">
                  <!--<el-option label="请选择" :value="0"></el-option>-->
                  <el-option label="分配到人"
                             :value="1"></el-option>
                  <el-option label="分配到项目"
                             :value="2"></el-option>
                </el-select>
              </el-form-item>
            </el-col>
            <el-col :span="12"
              ...
              <el-form-item label="人员或项目">
                <el-row>
                  <el-col :span="19">
                    <el-input readonly="readonly"
                              v-model="auditWarningSave.distributionName" />
                  </el-col>
                  <el-col :span="2"
                          :offset="1">
                    <el-button type="primary"
                               @click="distributionButtonClick">选择</el-button>
                  </el-col>
                </el-row>
              </el-form-item>
            </el-col>
          ...
          <el-form-item label="关联模型"
                        v-if="temp.warningType == 1">
            <el-row>
              <el-col align="right">
                <el-link type="primary"
                         v-if="!allReadOnly"
                         @click="selectModelVisible = true">选择模型</el-link>
              </el-col>
            </el-row>
            <el-table key="modelList"
              ...
              <el-table-column v-if="!allReadOnly"
                               label="操作"
                               prop="auditItemName"
                               align="center">
                <template slot-scope="scope">
          ...
          <el-form-item label="关联指标常用分析"
                        v-if="temp.warningType == 2">
            <el-row>
              <el-col align="right">
                <el-link type="primary"
                         v-if="!allReadOnly"
                         @click="selectIndicatorVisble = true">选择指标常用分析</el-link>
              </el-col>
            </el-row>
            <el-table key="indexList"
                      ref="indexListTable"
                      align="center"
                      :data="temp.indexList"
                      border
                      fit
                      highlight-current-row>
              <el-table-column prop="oftenindicatorsName"
                               label="指标常用分析名称"
                               width="300">
              </el-table-column>
              ...
              <el-table-column v-if="!allReadOnly"
                ...
                <template slot-scope="scope">
                  <el-link type="primary"
                           @click.native.prevent="deleteCommonlyAnalysis(scope.$index, temp.indexList)">删除
                  </el-link>
                </template>
              </el-table-column>
            </el-table>
      ...
      <el-tab-pane label="执行信息"
                   name="executeInfo">
        <el-form ref="executeDataForm"
                 :rules="rules"
                 class="detail-form"
                 :model="temp"
                 :disabled="allReadOnly">
          <el-form-item label="执行方式">
            <el-select v-model="temp.executeMode"
                       placeholder="请选择执行方式"
                       style="width:100%">
              <el-option label="单次执行"
                         :value="1"></el-option>
              <el-option label="多次执行"
                         :value="2"></el-option>
              <el-option label="周期执行"
                         :value="3"></el-option>
            </el-select>
          </el-form-item>
          <!--单次执行时间选择-->
          <el-form-item label="执行时间"
                        v-if="temp.executeMode == 1"
                        prop="singleExecuteTime">
            <el-date-picker v-model="temp.singleExecuteTime"
                            type="datetime"
                            placeholder="选择执行时间"
                            align="right"
                            style="width:100%"
          ...
          <!--多次执行时间选择-->
          <el-form-item label="多次执行时间"
                        v-if="temp.executeMode == 2">
            <el-form-item label=""
                          v-for="(domain, index) in temp.manyTimesExecuteTime"
                          :prop="'manyTimesExecuteTime.' + index + '.executeTime'"
                          :rules="rules.manyTimesExecuteTime.executeTime">
              ...
              <el-link type="primary"
                       v-if="index==0 && option !== 'detail'"
                       @click="addExecuteTime">添加&nbsp;</el-link>
              <el-link type="primary"
                       v-if="temp.manyTimesExecuteTime.length > 1 && option !== 'detail'"
                       @click="removeExecuteTime(domain)">删除</el-link>
              <!--<el-button type="primary" v-if="temp.manyTimesExecuteTime.length > 1" @click="removeExecuteTime(domain)">删除</el-button>-->
            </el-form-item>
          </el-form-item>
          <!--周期执行时间配置选择-->
          <el-form-item label=""
                        v-if="temp.executeMode == 3">
            <el-form-item label="执行周期设置">
              <el-select v-model="temp.intervalExecuteTime.intervalType"
                         placeholder="请选择执行周期"
                         style="width:100%">
                <el-option label="每天"
                           :value="1"></el-option>
            ...
            <!--周期执行每周-->
            <el-form-item label=""
                          v-if="temp.intervalExecuteTime.intervalType == 2">
            ...
            <!--周期执行每月-->
            <el-form-item label=""
                          v-for="(domain, index) in temp.intervalExecuteTime.monthValue"
                          v-if="temp.intervalExecuteTime.intervalType == 3">
              第 <el-input-number v-model="domain.value"
                               :min="1"
                               :max="31">
              </el-input-number> 天
              <el-link type="primary"
                       v-if="index==0 && !allReadOnly"
                       @click="addMonth">添加 &nbsp;</el-link>
              <el-link type="primary"
                       v-if="temp.intervalExecuteTime.monthValue.length > 1  && !allReadOnly"
                       @click="removeMonth(domain)">删除</el-link>
            </el-form-item>
            <!--周期执行每年-->
            <el-form-item label=""
                          v-for="(domain, index) in temp.intervalExecuteTime.monthDayValue"
                          v-if="temp.intervalExecuteTime.intervalType == 4">
              第 <el-select v-model="domain.month"
                         placeholder="请选择执行周期">
                <el-option label="1月"
                           :value="1"></el-option>
                <el-option label="2月"
              ...
              第 <el-input-number v-model="domain.day"
                               :min="1"
                               :max="31">
              </el-input-number> 天
              <el-link type="primary"
                       v-if="index==0 && !allReadOnly"
                       @click="addMonthDay">添加&nbsp;</el-link>
              <el-link type="primary"
                       v-if="temp.intervalExecuteTime.monthDayValue.length > 1  && !allReadOnly"
                       @click="removeMonthDay(domain)">删除</el-link>
            </el-form-item>
            <el-form-item label="执行时间设置"
                          prop="intervalExecuteTime.executeTime"
                          :rules="rules.singleExecuteTime">
              <el-time-picker v-model="temp.intervalExecuteTime.executeTime"
                              format='HH:mm'
                              value-format="HH:mm"
                              :picker-options="{
            ...
            <el-form-item label="开始结束日期"
                          v-show="false">
              <el-date-picker :picker-options="executeTimeOptions"
          ...
          <el-form-item label="模型参数配置"
            ...
            <el-row v-for="(domain,index) in temp.modelList"
                    :key="index">
              <h3 v-if="typeof  domain.paramObj !== 'undefined' && domain.paramObj.length > 0"
                  align="middle">{{domain.modelName}}</h3>
              <paramDrawNew v-if="typeof  domain.paramObj !== 'undefined' && domain.paramObj.length > 0"
                            :ref="'paramDrawRef'+domain.modelUuid"
                            :sql="paramSql[index]"
                            :arr="paramArr[index]"></paramDrawNew>
            </el-row>
    ...
    <el-dialog title="请选择指标常用分析"
      ...
      <span slot="footer"
        ...
        <el-button type="primary"
                   @click="getCommonlyAnalysis()">确 定</el-button>
      </span>
    </el-dialog>
    <el-dialog title="请选择人员"
      ...
      <span slot="footer"
        ...
        <el-button type="primary"
                   @click="getCheckPerson">确 定</el-button>
      </span>
    </el-dialog>
    <el-dialog title="请选择项目"
               ...
               :append-to-body="true"
               :close-on-click-modal="false">
      <userProject v-if="projectDialog"
                   ref="userproject"></userProject>
      <span slot="footer"
            class="dialog-footer">
        <el-button @click="projectDialog = false">取 消</el-button>
        <el-button type="primary"
                   @click="getCheckProject">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
