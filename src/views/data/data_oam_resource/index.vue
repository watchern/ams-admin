<template>
  ...
  <div class="page-container">
    <div id="leftPart" class="left-part">
      <el-input v-model="filterText" placeholder="输入关键字进行树的过滤" class="tree-search" />
      <MyElTree
          ref="tree"
          :data="data"
          :props="defaultProps"
        ...
        <el-table-column label="校验名称"
          ...
          <template slot-scope="scope">
            <el-button type="text"
                       ...
                       @click="details(scope.row.oamUuid)">{{scope.row.checkName}}</el-button>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    add () {
      ...
      if(this.form.parentUuid == null || this.form.parentUuid == undefined || this.form.parentUuid == ""){
        this.$message({type:"info",message:"请先选择树节点再进行新增"});
        return;
      }

      if(this.form.parentUuid == 0 ){
        this.$message({type:"info",message:"请先选择树的第三级子节点进行新增"});
        return;
      }

      if (this.form.path.split("/")[2]== null || this.form.path.split("/")[2] == undefined || this.form.path.split("/")[2] == ""){
        this.$message({type:"info",message:"请先选择树的第三级子节点进行新增"});
        return;
      }
