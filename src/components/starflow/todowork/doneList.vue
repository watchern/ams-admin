      ...
      <el-tab-pane label="已办列表" name="doneList">
          ...
          <el-collapse-transition>
            <div class="content" v-show="disp">
              <div class="content_form">
                <el-form ref="form" :model="queryDoneForm" label-width="128px" size="mini">
                  <el-row :gutter="20">
                    <el-col :span="8">
                      <el-form-item label="标题" class="form_item" prop="applyTitle">
                        <el-input
                          v-model="queryDoneForm.applyTitle"
                          :title="queryDoneForm.applyTitle"
                          style="width:180px;"
                        ></el-input>
                      </el-form-item>
                    </el-col>
                    <el-col :span="8">
                      <el-form-item label="审批类型" class="form_item" prop="applyTypeName">
                        <el-input
                          v-model="queryDoneForm.applyTypeName"
                          :title="queryDoneForm.applyTypeName"
                          style="width:180px;"
                        ></el-input>
                      </el-form-item>
                    </el-col>
                    <!-- <el-col :span="8">
                      <el-form-item label="处理结果" class="form_item" prop="applyState">
                        <el-select v-model="queryDoneForm.applyState" placeholder="请选择">
                          <el-option
                            v-for="(item,i) in applyStateOption"
                            :key="i"
                            :label="item.label"
                            :value="item.value"
                          ></el-option>
        ...
        <div class="table_template">
          ...
          >
            ...
            <el-table-column prop="thruDate" label="完成时间" show-overflow-tooltip></el-table-column>
            <el-table-column prop="applyState" label="处理结果" show-overflow-tooltip>
              <template slot-scope="scope">
                <span>{{dict.toName("auditStatus",scope.row.applyState)}}</span>
              </template>
            </el-table-column>
            <el-table-column prop="flowTrack" label="流程跟踪" show-overflow-tooltip width>
              <template slot-scope="scope">
                <el-link
            ...
            <!-- <el-table-column label="操作" fixed="right" show-overflow-tooltip>
              <template slot-scope="scope" >
                <div class="btn_action_img_box"  v-if="scope.row.createPersonUuid==scope.row.loginPersonUuid && scope.row.applyState==1" >
                  <el-button class="btn_editor" @click="takeBack(scope.row)">收回</el-button>
                  <el-button class="btn_editor" @click="stopSuspend(scope.row)">终止</el-button>
                </div>
              </template>
            </el-table-column>-->
          </el-table>
        ...
        <el-dialog title="详细跟踪【未完成】" :visible.sync="viewAppInfo" v-if="viewAppInfo" width="80%">
          <div>
            <todoDetail :applyUuid="applyUuid"></todoDetail>
          </div>
          <span slot="footer">
            <el-button size="mini" type="info" class="table_header_btn" @click="doneFlow = false">关闭</el-button>
          </span>
        </el-dialog>
        <el-dialog title="流程跟踪" :visible.sync="doneFlow" v-if="doneFlow" width="80%">
          <div>
        ...
        <template>-->
        <!-- 审前调查小结 -->
        <!-- <div v-if="applyDetail.type =='preInvest'" >
            <preInvestEdit
              ref="preInvestEdit"
              :versionUuid="flowItemParam.versionUuid"
              :appDataUuid="this.flowItemParam.appDataUuid"
            ></preInvestEdit>
          </div>
        </template>
        </div>-->
...
export default {
  ...
  methods: {
    ...
    todo(row) {
      ...
      this.flowItem.wftype = this.dict.flowValueFun(row.applyType);
      this.flowItem.temp1 = row.temp1;
      // localStorage.setItem("approvalData", JSON.stringify(row));
      // localStorage.setItem("applyDetail", JSON.stringify(this.applyDetail));
      // localStorage.setItem("flowSet", JSON.stringify(this.flowSet));
      // localStorage.setItem("flowItem", JSON.stringify(this.flowItem));
      ...
      //   name: "approvalSucce",
      //   query: {
      //     approvalData: JSON.stringify(row),
      //     applyDetail: JSON.stringify(this.applyDetail),
      //     flowSet: JSON.stringify(this.flowSet),
      //     flowItem: JSON.stringify(this.flowItem)
      //   }
      // });
