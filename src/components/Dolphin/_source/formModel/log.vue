        ...
        <div class="log-box" >
          <div class="title">
            ...
            <div class="full-screen">
              <a href="javascript:" @click="_downloadLog" data-container="body" data-toggle="tooltip" title='Download Log'>
                <em class="ans-icon-download" style="font-size: 20px"></em>
              </a>
              <a href="javascript:" class="refresh-log" :class="loading ? 'active' :''" @click="!loading && _refreshLog()" data-container="body" data-toggle="tooltip" title='Refresh Log'>
                <em class="ans-icon-refresh"></em>
              </a>
              <a href="javascript:" @click="_screenOpen" v-show="!isScreen" data-container="body" data-toggle="tooltip" title='Enter full screen'>
                <em class="ans-icon-max"></em>
              </a>
              <a href="javascript:" @click="_screenClose" v-show="isScreen" data-container="body" data-toggle="tooltip" title='Cancel full screen'>
          ...
          <div class="operation">
            <x-button type="primary" shape="circle" @click="close"> 'Close'</x-button>
          </div>
        </div>
      </div>
    </transition>
      ...
      _refreshLog () {
        ...
        this.store.dispatch('dag/getLog', this._rtParam).then(res => {
          setTimeout(() => {
            this.loading = false
            if (res.data) {
              this.$message.success('Update log success')
            } else {
              this.$message.warning('No more logs')
            }
          }, 1500)
          // Handling text field size
          handerTextareaSize().html('').text(res.data || 'No log')
        }).catch(e => {
      ...
      _ckLog () {
        ...
        this.store.dispatch('dag/getLog', this._rtParam).then(res => {
          ...
          if (!res.data) {
            ...
            // Handling text field size
            handerTextareaSize().html('').text(content || 'No log')
          } else {
            this.isData = true
            content = res.data
            // Handling text field size
