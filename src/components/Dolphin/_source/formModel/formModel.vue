...
<template>
  <div class="form-model-model" v-clickoutside="_handleClose">
    <div class="title-box">
      <span class="name">'Current node settings'</span>
      <span class="go-subtask">
        <!-- Component can't pop up box to do component processing -->
        <m-log :item="backfillItem">
          <template slot="history"><a href="javascript:" @click="_seeHistory" ><em class="ansicon ans-icon-timer"></em><em>'View history'</em></a></template>
          <template slot="log"><a href="javascript:"><em class="ansicon ans-icon-log"></em><em>'View log'</em></a></template>
        </m-log>
        <a href="javascript:" @click="_goSubProcess" v-if="_isGoSubProcess"><em class="ansicon ans-icon-node"></em><em>'Enter this child node'</em></a>
      </span>
    </div>
    <div class="content-box" v-if="isContentBox">
      <div class="from-model">
        ...
        <div class="clearfix list">
          <div class="text-box"><span>'Node name'</span></div>
          <div class="cont-box">
        ...
        <!-- Running sign -->
        <div class="clearfix list">
          <div class="text-box"><span>'Run flag'</span></div>
          <div class="cont-box">
            <label class="label-box">
              <x-radio-group v-model="runFlag" >
                <x-radio :label="'NORMAL'" :disabled="isDetails">'Normal'</x-radio>
                <x-radio :label="'FORBIDDEN'" :disabled="isDetails">'Prohibition execution'</x-radio>
              </x-radio-group>
            </label>
          </div>
        </div>
        ...
        <div class="clearfix list">
          ...
          <div class="cont-box">
            ...
            <span class="text-b">'Worker group'</span>
        ...
        <div class="clearfix list" v-if="taskType !== 'SUB_PROCESS'">
          <div class="text-box">
            <span>'Number of failed retries'</span>
          </div>
          <div class="cont-box">
            <m-select-input v-model="maxRetryTimes" :list="[0,1,2,3,4]">
            </m-select-input>
            <span>('Times')</span>
            <span class="text-b">'Failed retry interval'</span>
        ...
        <div class="clearfix list" v-if="taskType === 'CONDITIONS'">
          ...
          <div class="cont-box">
            ...
            <span class="text-b" style="padding-left: 38px">'Branch flow'</span>
    ...
    <div class="bottom-box">
      <div class="submit" style="background: #fff;">
        <x-button type="text" @click="close()"> 'Cancel' </x-button>
        <x-button type="primary" shape="circle" :loading="spinnerLoading" @click="ok()" :disabled="isDetails">{{spinnerLoading ? 'Loading...' : 'Confirm add' }}</x-button>
      </div>
    </div>
  </div>
</template>
    ...
    methods: {
      ...
      _goSubProcess () {
        if (_.isEmpty(this.backfillItem)) {
          this.$message.warning(`${'The newly created sub-Process has not yet been executed and cannot enter the sub-Process'}`)
          return
        }
        if (this.router.history.current.name === 'projects-instance-details') {
          let stateId = $(`#${this.id}`).attr('data-state-id') || null
          if (!stateId) {
            this.$message.warning(`${'The task has not been executed and cannot enter the sub-Process'}`)
            return
          }
      ...
      _verifName () {
        if (!_.trim(this.name)) {
          this.$message.warning(`${'Please enter name (required)'}`)
          return false
        }
        if (this.successBranch !='' && this.successBranch !=null && this.successBranch == this.failedBranch) {
          this.$message.warning(`${'Cannot select the same node for successful branch flow and failed branch flow'}`)
          return false
        }
      ...
      _verifWorkGroup() {
        ...
        if(item==undefined) {
          this.$message.warning(`${'The Worker group no longer exists, please select the correct Worker group!'}`)
          return false;
        }
        return true
      },
