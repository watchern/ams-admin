...
function main(){
	echo "Welcome to theams front-end deployment script, which is currently only supported by front-end deployment scripts : CentOS and Ubuntu"
	echo "Please execute in the ams-ams-admin directory"

	#To be compatible with MacOS and Linux
	if [[ "$OSTYPE" == "darwin"* ]]; then
    		# Mac OSX
    		echo "ams-admin install not support Mac OSX operating system"
    		exit 1
		...
		echo "linux"
    		...
    		echo "ams-admin not support Windows operating system"
    		exit 1
	...
	read -p "Please enter the nginx proxy port, do not enter, the default is 8898 :" esc_proxy_port
